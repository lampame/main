(function () {
    ...
    function Card(data) {
      var item = Lampa.Template.get('vinyl_card', {
        ...
        subtitle: data.subtitle || data.artist || data.language || ''
      });
    ...
    /**
     * Парсинг ответа JioSaavn API
     * JioSaavn возвращает JSON после префикса `/*-secure-`
     * @param {string} raw
     * @returns {object|null}
     */
    Api.parseResponse = function (raw) {
      if (!raw) return null;
      try {
        // Убираем префикс безопасности JioSaavn
    ...
    function normalizeSong(song) {
      return {
        ...
        subtitle: song.subtitle || song.primaryArtists || song.artist || '',
          ...
          this.append({
            ...
            results: data.albums.map(function (item) {
              return {
                id: item.id,
                title: item.title || item.name || '',
                subtitle: item.language || item.year || '',
                image: item.image || item.picture || '',
                type: 'album',
                media: 'album'
              };
      ...
      this.search = function (query, onResults) {
        ...
        function mapSong(song) {
          ...
          return {
            ...
            subtitle: song.primaryArtists || song.artist || song.subtitle || '',
            img: img,
            poster: img,
            image: img,
            id: song.id,
        ...
        function mapAlbum(album) {
          ...
          return {
            ...
            subtitle: album.artist || album.subtitle || album.year || '',
            img: img,
            poster: img,
            image: img,
            id: album.id,
        ...
        function mapPlaylist(pl) {
          ...
          return {
            title: pl.title || pl.name || 'Unknown',
        ...
        function mapArtist(artist) {
          ...
          return {
            title: artist.name || artist.title || 'Unknown',
